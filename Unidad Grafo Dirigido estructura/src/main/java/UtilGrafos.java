...
public class UtilGrafos {
    ...
    public static void imprimirMatrizMejorado(Double[][] matriz, Map<Comparable, TVertice> vertices, String titulo) {
    ...
    public static void imprimirMatrizWarshall(boolean[][] matriz, Map<Comparable, TVertice> vertices, String titulo) {
        if (vertices != null && matriz.length == vertices.keySet().size()) {
            Comparable[] etiquetas = vertices.keySet().toArray(new Comparable[vertices.keySet().size()]);
            int etiquetaMasLarga = stringMasLargo(etiquetas);
            int datoMasLargo = 0;
            ...
            for (int i = 0; i < matriz.length; i++) {
                ...
                for (int j = 0; j < matriz.length; j++) {
                    if (i == j) {
                        System.out.print(rellenar("Inf", largo, ' '));
                    } else {
                        System.out.print(rellenar(String.valueOf(matriz[i][j]), largo, ' '));
                    }
                }
