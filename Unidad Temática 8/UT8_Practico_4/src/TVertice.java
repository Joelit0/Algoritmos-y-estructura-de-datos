...
public class TVertice<T> implements IVertice {
    ...
    public void puntosArticulacion (LinkedList<TVertice> articulacion, int temp, TVertice anterior){
        setVisitado(true);
        articulacion.add(this);
        this.num_bp = temp;
        this.bajo = this.num_bp;
        for (TAdyacencia adyacente : adyacentes) {
            TVertice vertAdy = adyacente.getDestino();
            if (!vertAdy.getVisitado()) {
                vertAdy.puntosArticulacion(articulacion, temp+1, this);
                
            }
            if(vertAdy.num_bp < this.bajo && (anterior != vertAdy)){
                    this.bajo = vertAdy.num_bp;
            }
            
            if(this.num_bp < vertAdy.bajo){
                articulacion.add(vertAdy);
            }  
        }
    }
}
