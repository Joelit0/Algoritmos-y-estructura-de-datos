...
public class UtilGrafos {
    ...
    public static int[][] obtenerMatrizCostos(Map<Comparable, TVertice> vertices) {
        int cantidadVertices = vertices.size();
        int[][] matrizCostos = new int[cantidadVertices][cantidadVertices];
        ...
        for (int i = 0; i < matrizCostos.length; i++) {
            for (int j = 0; j < matrizCostos.length; j++) {
                if (i == j) {
                    ...
                    matrizCostos[i][j] = Integer.MAX_VALUE;
                }
            }
        }
        ...
        while (i < cantidadVertices) {
            ...
            while (j < cantidadVertices) {
                ...
                if (!elemVerticeI.getEtiqueta().equals(elemVerticeJ.getEtiqueta())) {
                    ...
                    int costoAdyacencia = verticeI.obtenerCostoAdyacencia(verticeJ);
    ...
    public static void imprimirMatrizMejorado(int[][] matriz, Map<Comparable, TVertice> vertices, String titulo) {
            ...
            for (int i = 0; i < matriz.length; i++) {
                ...
                for (int j = 0; j < matriz.length; j++) {
                    ...
                    if (matriz[i][j] == Integer.MAX_VALUE) {
                        ...
                        System.out.print(rellenar(String.valueOf(matriz[i][j]), largo, ' '));
                    }
                }
                System.out.println();
                System.out.println(linea);
    ...
    public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, boolean ignoreHeader, Class t  ) {
        ...
        for (String aristaString : aristas) {
            if ((aristaString != null) && (aristaString.trim() != "")) {
                ...
                aristasList.add(new TArista(datos[0], datos[1], Integer.valueOf(datos[2])));
            }
        }
